...
class Utility extends Model
{
    ...
    public static function settings()
    {
        ...
        if (\Auth::check()) {$data = Utility::getSetting();
        ...
        $settings = [
            ...
            'leave_status' => '1',
            "bug_prefix" => "#ISSUE",
            'title_text' => 'Alphainno ERP',
            'footer_text' => 'Alphainno ERP - Visa Consultancy Management System',
...
',
    ...
    /**
     * Resolve logo/favicon filename on disk (case-insensitive).
     */
    public static function resolveBrandFilename(?string $filename): ?string
    {
        if (empty($filename)) {
            return null;
        }
    ...
    /**
     * Resolve a publicly accessible URL for the company logo.
     */
    public static function companyLogoUrl(?string $filename = null): string
    {
        $filename = $filename
            ?: self::getValByName('company_logo_dark')
            ?: self::getValByName('company_logo_light')
            ?: 'logo.png';

        if (empty($filename)) {
            return asset('Alphainno.png');
        }

        $resolved = self::resolveBrandFilename($filename);
        if ($resolved) {
            $relative = 'uploads/logo/' . $resolved;

            return \Illuminate\Support\Facades\Storage::disk('public')->url($relative);
        }

        $legacyPath = storage_path('uploads/logo/' . ltrim($filename, '/'));
        if (file_exists($legacyPath)) {
            $publicDiskPath = storage_path('app/public/uploads/logo/' . basename($filename));
            \Illuminate\Support\Facades\File::ensureDirectoryExists(dirname($publicDiskPath));
            @copy($legacyPath, $publicDiskPath);

            return \Illuminate\Support\Facades\Storage::disk('public')->url('uploads/logo/' . basename($filename));
        }

        $helperUrl = self::get_file('uploads/logo');
        if (! empty($helperUrl)) {
            return rtrim($helperUrl, '/') . '/' . ltrim($filename, '/');
        }

        return asset('Alphainno.png');
    }

    public static function companyFaviconUrl(): string
    {
        $filename = self::getValByName('company_favicon') ?: 'favicon.png';

        return self::companyLogoUrl($filename);
    }

    /**
     * Stable cache-buster for logo/favicon URLs (changes only when files change).
     */
    public static function companyBrandVersion(): string
    {
        $candidates = array_filter([
            self::getValByName('company_logo_dark'),
            self::getValByName('company_logo_light'),
            self::getValByName('company_favicon'),
            'logo.png',
            'favicon.png',
        ]);
        ...
        foreach (array_unique($candidates) as $name) {
            $resolved = self::resolveBrandFilename($name);
            if (! $resolved) {
                continue;
            }
            $path = storage_path('app/public/uploads/logo/' . $resolved);
            if (file_exists($path)) {
                $times[] = filemtime($path);
            }
        }

        return $times ? (string) max($times) : '1';
    }
