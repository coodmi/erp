...
//                );
                $favicon = 'favicon.png';
                $path    = $request->file('favicon')->storeAs('uploads/logo', $favicon, 'public');
            }
    ...
    public function saveBusinessSettings(Request $request)
    {
        if(\Auth::user()->can('manage company settings'))
        {
            ...
            if ($request->hasFile('company_logo')) {
                $request->validate([
                    'company_logo' => 'image|mimes:png,jpg,jpeg|max:20480',
                ]);

                $logoName = 'logo.png';
                $request->file('company_logo')->storeAs('uploads/logo', $logoName, 'public');

                $creatorId = \Auth::user()->creatorId();
                foreach (['company_logo_dark', 'company_logo_light'] as $settingName) {
                    \DB::insert(
                        'insert into settings (`value`, `name`,`created_by`) values (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`) ',
                        [$logoName, $settingName, $creatorId]
                    );
                }
            }
            ...
            if ($request->hasFile('company_favicon')) {
                $request->validate([
                    'company_favicon' => 'image|mimes:png,jpg,jpeg,ico|max:20480',
                ]);

                $favicon = 'favicon.png';
                $request->file('company_favicon')->storeAs('uploads/logo', $favicon, 'public');

                $logoDir = storage_path('app/public/uploads/logo');
                if (is_dir($logoDir)) {
                    foreach (scandir($logoDir) as $file) {
                        if (preg_match('/^favicon\.(png|ico|jpg|jpeg)$/i', $file) && strcasecmp($file, $favicon) !== 0) {
                            @unlink($logoDir . DIRECTORY_SEPARATOR . $file);
                        }
                    }
                }
            ...
            $settings = Utility::settings();
            $textFields = ['title_text', 'footer_text', 'mail_from_address', 'company_telephone'];
            ...
            if (!empty($request->SITE_RTL) || !empty($request->default_language) || isset($request->display_landing_page) || isset($request->gdpr_cookie) || isset($request->color) || isset($request->cust_theme_bg) || isset($request->cust_darklayout)) {
                $post = $request->all();
                ...
                if (!isset($request->cust_theme_bg)) {
                    $post['cust_theme_bg'] = isset($request->cust_theme_bg) ? 'on' : 'off';
                }
                if (!isset($request->cust_darklayout)) {
                    $post['cust_darklayout'] = isset($request->cust_darklayout) ? 'on' : 'off';
                }
                if (isset($request->color) && $request->color_flag == 'false') {
                    $post['color'] = $request->color;
                } elseif ($request->filled('custom_color')) {
                    $post['color'] = $request->custom_color;
                }

                unset($post['_token'], $post['company_logo'], $post['company_logo_dark'], $post['company_logo_light'], $post['company_favicon'], $post['custom_color']);
