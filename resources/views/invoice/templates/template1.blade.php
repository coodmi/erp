@php
    ...
    // Determine if accent is dark → use white text, else dark text
...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .doc-table tfoot tr:first-child td {
...
<div class="doc-wrap" id="boxes">
    ...
    <div class="doc-body">
        ...
        <div class="doc-addresses">
            ...
            <div class="doc-address-box">
                ...
                @if(!empty($customer->shipping_name))
                    <strong>{{ $customer->shipping_name }}</strong>
                    <p>
        ...
        <div class="doc-status">
            <span>●</span> {{ __('Status') }}: {{ __($statusLabels[$invoice->status] ?? 'Unknown') }}
        </div>
        ...
        <div class="doc-totals-wrap">
            <div class="doc-totals">
                <div class="doc-totals-row">
                    ...
                    <span>{{ Utility::priceFormat($settings, $invoice->getSubTotal()) }}</span>
                </div>
                @if($invoice->getTotalDiscount())
                <div class="doc-totals-row">
                    <span>{{ __('Discount') }}</span>
                    <span>– {{ Utility::priceFormat($settings, $invoice->getTotalDiscount()) }}</span>
                </div>
                @endif
                ...
                <div class="doc-totals-row">
                    <span>{{ __('Paid') }}</span>
                    <span>{{ Utility::priceFormat($settings, ($invoice->getTotal() - $invoice->getDue()) - $invoice->invoiceTotalCreditNote()) }}</span>
                </div>
                @if($invoice->invoiceTotalCreditNote())
                <div class="doc-totals-row">
                    <span>{{ __('Credit Note') }}</span>
                    <span>{{ Utility::priceFormat($settings, $invoice->invoiceTotalCreditNote()) }}</span>
                </div>
                @endif
                <div class="doc-totals-row due">
                    <span>{{ __('Amount Due') }}</span>
                    <span>{{ Utility::priceFormat($settings, $invoice->getDue()) }}</span>
                </div>
            </div>
        </div>
