...
<body class="{{ $themeColor }}">
    <style>
        *, *::before, *::after { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Login page owns the viewport; reset only the shell, not form controls/icons inside */
        .auth-layout-shell {
            all: unset;
            display: block;
            width: 100%;
        ...
        header, footer, .auth-footer, .dash-header { display: none !important; }
    </style>
                ...
                <a class="navbar-brand" href="#">
                    @if($setting['cust_darklayout'] && $setting['cust_darklayout'] == 'on' )
                        <img src="{{ $logo . '/' . (isset($company_logos) && !empty($company_logos) ? $company_logos : 'logo-dark.png') }}"
                             alt="{{ config('app.name', 'Alphainno ERP') }}" class="logo">
                    @else
